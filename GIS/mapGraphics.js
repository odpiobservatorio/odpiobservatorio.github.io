...
function clearLayers() {
  ...
  const elemento =
    document.getElementById("LayerDepartamentos").parentElement.parentElement;
...
const allLayers = {
  ...
  LayerColorMap: () => {
    ...
    for (const registro of DataPrincipal) {
      const elemento = registro.Departamento.normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toUpperCase();
  ...
  LayerDepartamentos: () => {
    Layers["LayerDepartamentos"] = new L.geoJSON(capaDepartamentos, {
      ...
      filter: function (feature, layer) {
        if (Datafilter == 1) {
          return (
            feature.properties.NOMBRE_DPT == "CESAR" ||
            feature.properties.NOMBRE_DPT == "CHOCÓ"
          );
        } else {
          return feature.properties;
        }
  ...
  LayerBloquePretrolero: () => {
    Layers["LayerBloquePretrolero"] = new L.geoJSON(CapaBloquePetrolero, {
      ...
      filter: function (feature, layer) {
        return (
          feature.properties.TIPO_CONTR !== "NO APLICA" &&
          feature.properties.ESTAD_AREA !== "SIN ASIGNAR"
        );
      },
    })
      .bindPopup((layer) => {
...
function putLabel(e) {
  if (ActiveLabels == "1") {
    LabelMap = new L.marker(e.latlng, {
      draggable: "true",
      icon: otherIcons["senalador"],
    });
